...
$(document).ready(function () {
    function createCard(title, goal, progress) {
        ...
        valuesDiv.append(`<span class="c-title item">${title}</span>`)
        valuesDiv.append(`<span class="c-goal item">${goal}</span>`)
        valuesDiv.append(`<span class="c-progress item">${progress}</span>`)

        const progressPercentage = Math.round(Math.max(0, Math.min(progress, goal)) / goal * 100)
    ...
    // save an array to cookies
    function saveArrayToCookie(name, array) {
        $.cookie(name, JSON.stringify(array), {expires: 30})
    }
    ...
    // if cookies are not empty, load them
    if ($.cookie("titles") != null && getArrayFromCookie("titles") != null) {
        titles = getArrayFromCookie("titles")
        goals = getArrayFromCookie("goals")
        progresses = getArrayFromCookie("progresses")
        for (let i = 0; i < titles.length; i++) {
            createCard(titles[i], goals[i], progresses[i])
        }
    }
    ...
    addBtn.click(function () {
        ...
        // save the title, goal and progress in cookies
        titles.push(title.val())
        goals.push(goal.val())
        progresses.push(progress.val())
        saveArrayToCookie("titles", titles)
        saveArrayToCookie("goals", goals)
        saveArrayToCookie("progresses", progresses)

        // create a card
        createCard(title.val(), goal.val(), progress.val());
    ...
    clearBtn.click(function () {
        ...
        // remove all cookies
        $.removeCookie("titles", {expires: 30})
        $.removeCookie("goals", {expires: 30})
        $.removeCookie("progresses", {expires: 30})
        titles = []
        goals = []
        progresses = []
    })
